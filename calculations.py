...
def findMinMaxEtaXi(beta_val, b_val, sigmas_val, rho_val, a_val, xis_val, mu_k_val):
    sigma_space = np.linspace(sigmas_val[0], sigmas_val[1], precision_max_eta)
    xi_space = np.linspace(xis_val[0], xis_val[1], precision_max_eta)
    func_vals = np.array([funcXi(beta_val, xi_val, mu_k_val) -b_val*eta(beta_val, sigma_val, rho_val, a_val) for sigma_val in sigma_space for xi_val in xi_space])
    idx = np.argmin(func_vals)
    sigma_star = sigma_space[idx // precision_max_eta]
    xi_star = xi_space[idx % precision_max_eta]
    return sigma_star, xi_star
...
def longTermGrowth(beta_val, mu_vals, mu_k_vals, sigma_k_val, xi_vals, rho_vals, a_val, b_val, sigma_vals):
    ...
    sigma_star, xi_star = findMinMaxEtaXi(beta_val, b_star, sigma_vals, rho_star, a_star, xi_vals, mu_k_star)
